...
# Here we have 100 evenly spaced numbers from 0 to 1.
y = x + np.random.normal(scale=0.1, size=x.shape)
# Here we draw random samples from a normal (Gaussian) distribution.
...
def mse(y: np.ndarray, y_hat: np.ndarray) -> float:
    # implement me!
    # your_code
    return 1/(y.size) * np.sum(np.square(y-y_hat))
...
def optimize(x: np.ndarray, y: np.ndarray, a: float, b: float, learning_rate: float = 0.1):
    ...
    # your_code
    N = y.size
    # Parameter update - as shown above 
...
# fix: sparse_output instead of sparse 
...
learning_rate = 1e-3
# X_tensor: shape (N, D) -> N - number of samples, D - number of features
# number of features in 1 sample -> X_train.shape[1]
# out_features = 1, because we have 1 output - logit
...
class MLP(nn.Module):
    def __init__(self, input_size: int):
        ...
        # implement me!
        # your_code
        self.model = nn.Sequential(nn.Linear(input_size, 256), nn.ReLU(), nn.Linear(256, 128), nn.ReLU(), nn.Linear(128, 1))
...
def evaluate_model(model: nn.Module, X: torch.Tensor, y: torch.Tensor, loss_fn: nn.Module, threshold: Optional[float] = None) -> Dict[str, float]:
    # implement me!
    # your_code
    # Setting our model to evaluation model
    model.eval()
    with torch.no_grad():  
        # Prediction (logits)
        ...
        # Optimal threshold (implementation from task 3)
...
class RegularizedMLP(nn.Module):
    def __init__(self, input_size: int, dropout_p: float = 0.5):
        ...
        # implement me!
        # your_code
        self.model = nn.Sequential(nn.Linear(input_size, 256), nn.ReLU(), nn.Dropout(p=dropout_p), 
                                   nn.Linear(256, 128), nn.ReLU(), nn.Dropout(p=dropout_p), 
                                   nn.Linear(128, 1))
...
for epoch_num in range(max_epochs):
    ...
    if steps_without_improvement >= early_stopping_patience:
        print(f"Early stopping detected after {epoch_num} epochs")
        break
...
class NormalizingMLP(nn.Module):
    def __init__(self, input_size: int, dropout_p: float = 0.5):
        ...
        # implement me!
        # your_code
        self.mlp = nn.Sequential(nn.Linear(input_size, 256), nn.BatchNorm1d(256), nn.ReLU(), nn.Dropout(p=dropout_p), 
                                 nn.Linear(256, 128), nn.BatchNorm1d(128), nn.ReLU(), nn.Dropout(p=dropout_p), 
                                 nn.Linear(128, 1))
...
positive_weight = torch.tensor(class_weights[1])
