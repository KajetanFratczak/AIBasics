...
print(sys.version)
# nie działało mi uv na 3.13.2, więc jest na 3.12
...
# removal by negation - interesting approach
...
# I also do it using loc, negation, and the greater than method
...
# Fireplaces
# FireplaceQu : data description says NA means "no fireplace", więc przyjmuje że mam 0 domyślnie
...
# converting categorical data to binary numbers
categorical_pipeline = Pipeline(
    steps = [
        ('categorical_trans', one_hot_encoder)
    ]
)  

# filling missing numerical data and scaling them
numerical_pipeline = Pipeline(
    steps = [
        ('numerical_compl', median_imputer),
        ('numerical_trans', min_max_scaler)
    ]
) 
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Uważam, że następuje przeuczenie modelu ze względu na subiektywnie dużą różnicę między wynikami.
# Błąd treningowy jest niższy od testowego aż o ok. 4500 $, co względem wyników jest dość duże.
# Model musimy zatem regularyzować (gdybyśmy nie musieli to po co by była następna sekcja ;) )
#
...
# 1) RidgeCV - I check 1000 values [0.1, 100] on a linear scale (linspace())
reg_ridge = RidgeCV(alphas=np.linspace(0.1, 100.0, num=1000))
reg_ridge.fit(X_train,y_train)

# 2) LassoCV - number of alpha values = 1000, 5-fold CV, random_state=0
reg_lasso = LassoCV(alphas=1000, cv=5, random_state=0)
reg_lasso.fit(X_train,y_train)

# 3) Print the found optimal regularization strength values .alpha_ for each, rounded to 4 decimal places for readability.
reg_ridge_alpha = reg_ridge.alpha_
reg_lasso_alpha = reg_lasso.alpha_
print(f"reg_ridge_alpha: {reg_ridge_alpha:.4f}")
print(f"reg_lasso_alpha: {reg_lasso_alpha:.4f}")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# W obu przypadkach udało się wyeliminować overfitting, choć w przypadku LassoCV udało się to lepiej.
#
...
# %%
# deleting 4 columns: default, duration, pdays, poutcome
df = df.drop(["default", "duration", "pdays", "poutcome"], axis="columns")

# %%
# deleting rows where education level is illiterate
df = df.loc[~df["education"].isin(["illiterate"]), :]

# %%
# encoding the variables: education, contact, month, day_of_week and y accordingly
df = df.replace (
    {
        "education": {
            "basic.4y": "primary",
...
# %%
# extracting the 'y' column into the variable `y` (make sure to remove it from the DataFrame)
y = df["y"]
df = df.drop("y", axis=1)
...
ax = counts.plot.bar(title="Classes Frequency")
plt.xlabel("Class")
...
numerical_pipeline = Pipeline(
    steps = [
        ('numerical_trans', standard_scaler)
    ]
)

# The order of processing categorical -> numerical and numerical -> categorical matters here
# It was a small bug I struggled with
column_transformer = ColumnTransformer(
    transformers = [
        ('categorical', categorical_pipeline, categorical_features),
        ('numerical', numerical_pipeline, numerical_features),
...
# 1. Training a logistic regression model without regularization
nol_reg_logistic = LogisticRegression(class_weight="balanced", penalty=None, random_state=0)
nol_reg_logistic.fit(X_train, y_train)

y_pred = nol_reg_logistic.predict(X_test)

nol_precision = precision_score(y_test, y_pred)
nol_recall = recall_score(y_test, y_pred)
nol_f1 = f1_score(y_test, y_pred)

print(f"Model bez regularyzacji")
print(f"Precyzja: {nol_precision*100:.2f}%")
print(f"Czułość: {nol_recall*100:.2f}%")
print(f"F1: {nol_f1*100:.2f}%")


# %%
# 2. Tuning the model with L2 regularization using LogisticRegressionCV
from sklearn.linear_model import LogisticRegressionCV

l2_reg_logistic = LogisticRegressionCV(class_weight="balanced", penalty="l2", Cs=100, cv=5, scoring="f1", random_state=0, n_jobs=-1)
l2_reg_logistic.fit(X_train, y_train)

y_pred = l2_reg_logistic.predict(X_test)

l2_precision = precision_score(y_test, y_pred)
l2_recall = recall_score(y_test, y_pred)
l2_f1 = f1_score(y_test, y_pred)

print(f"Model z regularyzacją L2")
print(f"Precyzja: {l2_precision*100:.2f}%")
print(f"Czułość: {l2_recall*100:.2f}%")
print(f"F1: {l2_f1*100:.2f}%")

# %%
# 3. Tuning the model with L1 regularization using LogisticRegressionCV
from sklearn.linear_model import LogisticRegressionCV

l1_reg_logistic = LogisticRegressionCV(class_weight="balanced", penalty="l1", Cs=100, cv=5, scoring="f1", random_state=0, n_jobs=-1, solver="saga")
l1_reg_logistic.fit(X_train, y_train)

y_pred = l1_reg_logistic.predict(X_test)

l1_precision = precision_score(y_test, y_pred)
l1_recall = recall_score(y_test, y_pred)
l1_f1 = f1_score(y_test, y_pred)

print(f"Model z regularyzacją L1")
print(f"Precyzja: {l1_precision*100:.2f}%")
print(f"Czułość: {l1_recall*100:.2f}%")
print(f"F1: {l1_f1*100:.2f}%")
...
# %% editable=true slideshow={"slide_type": ""} tags=["ex"]
# your_code
# 4. Analiza wytrenowanych modeli 
# Oblicz miarę F1 na zbiorze treningowym modelu bez żadnej regularyzacji i porównaj go z wynikiem testowym; czy występuje tutaj overfitting?
# Wartości przypisz do zmiennych f1_train oraz f1_test.
# Czy twoim zdaniem tworzenie modeli z regularyzacją ma sens w tym przypadku?

# predicition
y_train_pred = nol_reg_logistic.predict(X_train)
y_test_pred = nol_reg_logistic.predict(X_test)

# calculating
f1_train = f1_score(y_train, y_train_pred)
f1_test = f1_score(y_test, y_test_pred)

print(f"Analiza overfittingu")
print(f"F1 na zbiorze testowym: {f1_test*100:.2f}%")
print(f"F1 na zbiorze treningowym: {f1_train*100:.2f}%")
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# W podpunkcie 4 nie występuje overfitting, gdyż F1 na zbiorze testowym niewiele różni się od F1 na zbiorze treningowym. Model zatem radzi sobie tak samo dobrze na danych testowych jak radził sobie na danych treningowych.
# Czy tworzenie modeli z regularyzacją ma w tym przypadku sens? - Uważam, że nie, ponieważ regularyzacja nie poprawia wyników. 
# Co jest ważniejsze precision czy recall dla tego problemu? - Precyzja, ponieważ wysyłanie reklam do niezainteresowanych klientów kosztuje, więc chcemy zmaksymalizować zarobki na realnie zainteresowanych klientach.
# Jak zmienić ich stosunek nie zmieniając modelu? - Możemy zmienić próg klasyfikacji (decision threshold) - domyślnie jest to 0.5, można np. podnieść na 0.8. Można też zmienić parametr scoring (metrykę optymalizacyjną) na "precision".
...
# numerical data with polynomial features
numerical_pipeline = Pipeline(
    steps = [
        ('numerical_trans', standard_scaler),
        ('polynomial_trans', polynomial_features)
    ]
)
...
# %%
# 2. Wytrenuj model regresji logistycznej bez regularyzacji na takim powiększonym zbiorze.
nol_reg_poly_logistic = LogisticRegression(class_weight="balanced", penalty=None, random_state=0, max_iter=1000)
nol_reg_poly_logistic.fit(X_train, y_train)

y_pred = nol_reg_poly_logistic.predict(X_test)

# prediction
y_poly_train_pred = nol_reg_poly_logistic.predict(X_train)
y_poly_test_pred = nol_reg_poly_logistic.predict(X_test)

# calculating
f1_train = f1_score(y_train, y_poly_train_pred)
f1_test = f1_score(y_test, y_poly_test_pred)

print(f"F1 na zbiorze testowym: {f1_test*100:.2f}%")
print(f"F1 na zbiorze treningowym: {f1_train*100:.2f}%")

# STOP: TOTAL NO. OF ITERATIONS REACHED LIMIT
# zatem dodam max_iter
...
# %% [markdown] editable=true slideshow={"slide_type": ""} tags=["ex"]
# Czy jest sens tworzyć modele z regularyzacją? - Uważam, że nie ma sensu, ponieważ jak możemy zobaczyć powyżej zjawisko overfittingu praktycznie nie występuje. Ponadto, dodanie cech wielomianowych nieznacznie ulepszyło nasz model, co możemy zobaczyć na podstawie wartości F1, więc problem naprawy F1 nie sprowadzi się w tym przypadku do regularyzacji, która jak już widzieliśmy niewiele wniosła, a jedynie skomplikowała proces trenowania modelu. 
#
